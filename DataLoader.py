...
class MachineDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        max_df_index = self.df[self.df['machineID'] == machine_id].index[-1]
        mix_df_index = self.df[self.df['machineID'] == machine_id].index[0]
        df_machine_has_failure = ((self.df['machineID'] == machine_id) &
                                  ((self.df['comp1'] == 1) | (self.df['comp2'] == 1) | (self.df['comp4'] == 1)))
        df_indexes_with_failure = self.df[df_machine_has_failure][['comp1', 'comp2', 'comp4']].index

        if len(df_indexes_with_failure) == 0:
            start = np.random.randint(0, len(self.df[self.df['machineID'] == machine_id])
                                      - self.training_window - self.forecast_window)
        else:
            p = np.random.exponential(2, len(df_indexes_with_failure))
            start_index_with_error = np.random.choice(df_indexes_with_failure, p=p / p.sum())
            if start_index_with_error + self.training_window + self.forecast_window > max_df_index:
                start = max_df_index - self.training_window - self.forecast_window - mix_df_index
            else:
                start = max((0, start_index_with_error - mix_df_index - self.training_window))
        ...
        index_target = tensor(
            [*range(start + self.training_window, start + self.training_window + self.forecast_window)])
        _input = tensor(
            self.df[self.df['machineID'] == machine_id][columns][start: start + self.training_window].values)
        target = tensor(self.df[self.df['machineID'] == machine_id][columns][
